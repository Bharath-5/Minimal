...
myWorkspaces = ["web","file","code","doc","gen","note","chat","misc"]
...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    -- launch qutebrowser
    , ((modm,               xK_F1    ), spawn "qutebrowser")

    -- launch pcmanfm
    , ((modm,               xK_F2    ), spawn "pcmanfm")

    -- launch libreoffice
    , ((modm,               xK_F4    ), spawn "libreoffice")

    -- launch joplin
    , ((modm,               xK_F6     ), spawn "/home/bharath/.cache/yay/joplin-appimg/Joplin*.AppImage")
...
myManageHook = composeAll
    ...
    , title =? "qutebrowser"        --> doShift ( "web" )
    , (className =? "firefox" <&&> resource =? "Dialog") --> doFloat
    , className =? "URxvt"          --> doShift ( "code" )
    , className =? "Pcmanfm"        --> doShift ( "file" )
    , className =? "libreoffice-startcenter"    --> doShift ( "doc" )
    , className =? "Joplin"       --> doShift ( "note" )
...
--
main = do
        xmproc <- spawnPipe "xmobar  /home/bharath/.config/xmobar/xmobar.config"
